...
import utils.{ErrorHandler, FuzzyNameMatcher}
...
class AssociationController @Inject()(
    ...
    req
      ...
      .map { loggedInAsPsa =>
        if (loggedInAsPsa) {
          req.psaId match {
            case Some(psaId) =>
              Right((psaId.id, psaId.name, "poda"))
            case None =>
              Left("PsaId credentials not found")
          }
        } else {
          req.pspId match {
            case Some(pspId) =>
              Right((pspId.id, pspId.name, "podp"))
            case None =>
              Left("PspId credentials not found")
          }
        }
      }
      .getOrElse(Left("No header present loggedInAsPsa [true, false] - Should indicate if user is logged in as a PSA or PSP"))

  private def withRegime(id: String, idType: String, regime: String, name: String)
                        (implicit req: actions.PsaAuthRequest[AnyContent]): Future[Result] =
    getMinimalDetail(id, idType, regime) map {
      case Right(minimalDetails) =>
        if (FuzzyNameMatcher.matches(minimalDetails.name.getOrElse(""), name)) {
  ...
  private def withParameters(id: String, idType: String, name: String)
                            (implicit req: actions.PsaAuthRequest[AnyContent]): Future[Result] =
    if (idType == "psaid") {
      withRegime(id, idType, regime = "poda", name)
    } else if (idType == "pspid") {
      withRegime(id, idType, regime = "podp", name)
    } else {
      Future.successful(BadRequest("idType must be either psaid or pspid"))
    }

  def getEmailInvitation(srn: SchemeReferenceNumber): Action[AnyContent] =
    (psaAuth andThen schemeAuth(srn)).async { implicit request =>
      (
        request.headers.get("id"),
        request.headers.get("idType"),
        request.headers.get("name")
      ) match {
        case (Some(id), Some(idType), Some(name)) =>
          withParameters(id, idType, name)
        case (id, idType, name) =>
          def printMissingParam(paramName: String, param: Option[String]) = if (param.isEmpty) " " + paramName else ""

          val missingParamErrors =
            Seq("id" -> id, "idType" -> idType, "name" -> name).map {
              case (name, param) => printMissingParam(name, param)
            }.mkString

          Future.successful(BadRequest(s"Missing headers:$missingParamErrors"))
      }
    }
  ...
  def acceptInvitation: Action[AnyContent] = psaPspAuth.async {
    implicit request =>
      request
        ...
        .asJson
        .map { json =>
          json.validate[AcceptedInvitation] match {
            case JsSuccess(acceptedInvitation, _) =>
              associationConnector.acceptInvitation(acceptedInvitation).map {
                case Right(_) => Created
                case Left(e) => result(e)
              }
            case JsError(errors) =>
              Future.failed(new BadRequestException("Bad request received from frontend for accept invitation"))
          }
        }
        .getOrElse(Future.failed(new BadRequestException("No Request Body received for accept invitation")))
  }
    ...
    minimalDetailsCacheRepository.get(idValue).flatMap {
      case Some(response) =>
        response.validate[MinimalDetails](using MinimalDetails.defaultReads) match {
          ...
          case JsError(_) =>
            getAndCacheMinimalDetails(idValue, idType, regime)
        }
      case _ =>
        getAndCacheMinimalDetails(idValue, idType, regime)
    }
  ...
  private def getAndCacheMinimalDetails(idValue: String, idType: String, regime: String)
    ...
    associationConnector.getMinimalDetails(idValue, idType, regime).flatMap {
      case Right(psaDetails) =>
        minimalDetailsCacheRepository
          .upsert(idValue, Json.toJson(psaDetails))
