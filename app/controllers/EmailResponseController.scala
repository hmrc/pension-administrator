...
class EmailResponseController @Inject()(
  ...
  def retrieveStatus(journeyType: EmailJourneyType.Value, id: String): Action[JsValue] = Action(BodyParsers.parse.tolerantJson) {
    implicit request =>
          ...
          request.body.validate[EmailEvents].fold(
            ...
            valid => {
              valid.events.filterNot(
                ...
                println(s"\n\n\n\n\nEmail Audit event coming from $journeyType is $event")
